...
public class SlushSnowflake {
   ...
   * random.
   * <p>
   * We now examine how the phase shift point behaves with increasing k. Whereas k = 1 is optimal
   * for Slush (i.e. the network topples the fastest when k = 1), we see that Snowflake’s safety is
   * maximally displaced from its ideal position for k = 1, reducing the number of feasible
   * solutions. Luckily, small increases in k have an exponential effect on sps, creating a larger
   * set of feasible solutions. More formally:
   */
  private static final int K = 7;

  /**
   * it checks if a fraction ≥ αk are for the same color, where α > 0.5 is a protocol parameter.
   */
  private static final int AK = 4;

  /**
   * A node accepts the current color when its counter exceeds β
   */
  ...
  void stats() {
    ...
    for (SlushNode n : nodes) {
      if (n instanceof ByzantineNode)
        byz++;
      else if (n instanceof SnowflakeNode)
        bft++;
      else
        std++;
